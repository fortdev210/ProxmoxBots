...
class PuppeteerBase {
  constructor() {
    ...
    this.default_page_width = 1920;
    this.default_page_height = 1080;
  }
  ...
  async openNewPage() {
    ...
    await page.setViewport({
      width: this.default_page_width + Math.floor(Math.random() * 100),
  ...
  async resolveCaptcha() {
    ...
    while (captchaDetected && i < 3) {
      i++;
      const frames = this.page.frames();
      const captchaFrame = frames[frames.length - 1];
      await captchaFrame.hover('div[role="main"]');
      await captchaFrame.focus('div[role="main"]');
      await captchaFrame.click('div[role="main"]', {
        delay: Math.random() * 15000 + 5000,
      });
      await captchaFrame.waitForTimeout(Math.random() * 5000 + 5000);
      console.log(`Try to solve captcha ${i} times`);
      captchaDetected = await this.checkCaptcha(10000);
    }
