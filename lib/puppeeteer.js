...
class PuppeteerBase {
  constructor() {
    ...
    this.default_page_width = 1500;
    this.default_page_height = 800;
  }
  ...
  async initPuppeteer() {
    const browser = await puppeteer.launch({
      ...
      args: [
        ...
        "--disable-extensions-except=" + parentDir + "/ext/dsh",
        "--load-extension=" + parentDir + "/ext/dsh",
      ],
    });
  ...
  async openNewPage() {
    ...
    page.on("request", (request) => {
      ...
      if (url.endsWith("init.js") || url.endsWith("captcha.js"))
        request.abort();
