...
class API_MANAGER {
  ...
  async putEmailInPrep(orderId, customerEmail) {
    var url = "http://admin.stlpro.com/v1/order_item/" + orderId + "/";
  ...
  async putInProcessingFlag(orderId) {
    var url = "http://admin.stlpro.com/v1/order_item/" + orderId + "/";
    LOGGER.info(`Moving order to Walmart Processing Flag`);
  ...
  async removeEmailFromPrep(orderId) {
  ...
  async addRegistryLink(orderId, registryLink) {
    ...
    try {
      ...
      LOGGER.error("Error while adding registry link to order.");
    }
  }
}
