...
class API_MANAGER {
  ...
  async getDsOrdersExtraItemToCancel(){ 
    let api = 'http://admin.stlpro.com/v2/ds_order/?buyer_name=&ds_status_filter=Purchased+Acknowledged+Confirmed&extra_items=Extra+Item&extra_items_canceled=NOT+canceled&finished=&not_found=&ordering=-ended_at&paid=&started_at_filter=Last+7+days&supplier=W'    
    let orders = [];
    try {
      while (api !== null) {
  ...
  async updateDeliveryState(data) {
    ...
    try {
      ...
      console.log("Error happened while updating the order status", error);
    }
  };
}
