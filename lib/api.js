...
class API_MANAGER {
  ...
  async fetchOrderItems(flag, az_account = "") {
    const link = `http://admin.stlpro.com/v2/order_item/?supplier=W&flag=${flag}&quantity_ordered=&order_status=New+Order&pii_data=&ship_service_level=&free_delivery=&az_account=${az_account}&box_size=`;
    try {
  ...
  async updatePurchasedOrderNumber(orderId, orderNumber, items) {
    const url = `https://admin.stlpro.com/v2/ds_order/${orderId}/finish/`;
    const data = {
      order_item_id: orderNumber,
      items: items,
    };
    try {
      const response = await axios.post(url, data, { headers: this.headers });
      LOGGER.info("Successfully submitted order number: ", orderNumber);
    } catch (error) {
      LOGGER.error("Error while submitting order number.", error);
    }
  }
}
