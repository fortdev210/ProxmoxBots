...
const hostName =
  process.env.TEST_PREPBOT === "true"
    ? "WM-PREP03"
    : process.env.TEST_BUYBOT
    ? "WM-BUYBOT03"
    : require("os").hostname();
...
class STLPRO_MANAGER extends PuppeteerBase {
  constructor(currentOrder) {
    ...
    this.botNumber = parseInt(hostName.replace(/[^0-9]+/g, ""));
  ...
  async getOrderDetails() {
    ...
    const registryLink = await this.page.evaluate(() => {
      return document.querySelectorAll('a[href*="walmart.com"]')[4].href;
    });
    ...
    customerInfoClean.registryLink = registryLink;
