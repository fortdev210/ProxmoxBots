...
class STLPRO_MANAGER extends PuppeteerBase {
  constructor() {
    ...
    // this.botNumber = hostName.replace(/[^0-9]+/g, "");
    this.botNumber = "01";
  }
  ...
  async getOrders() {
    ...
    await this.waitForLoadingElement('[id="itemsSearch"]');
    try {
  ...
  async getOrderDetails(currentOrder) {
    ...
    await this.insertValue('[id="orders"]', currentOrder);
    await this.clickButton('[value="Submit"]');
    await this.waitForLoadingElement('[id="updateAllItemsForm"]');
    ...
    customerInfo[9] = getCorrectState(customerInfo[9]);
    customerInfoClean = {};
    ...
    if (this.hostName.indexOf("REBUY") > -1) {
