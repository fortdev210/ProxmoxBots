...
class Flag_Order {
  ...
  async putInKelly() {
    console.log("Order flagged to Kelly.");
    await this.setRequestInstance();
    this.req.send(
      JSON.stringify({ note: "Ill processed order.", flag: "Kelly" })
    );
  }
  ...
  async setCantShipAddress() {
    await this.setRequestInstance();
    console.log("Moving order to Cant ship to this address");
    this.req.send(
      JSON.stringify({
        note: "Processed by wm buy bot",
        flag: "Cant Ship to Address",
      })
    );
  }

  async putInBuyer1Flag(registryLink) {
    await this.setRequestInstance();
