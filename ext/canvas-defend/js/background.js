...
const NotificationInfo = {
    newHash: { title: "Canvas Fingerprint Generator is active" },
...
chrome.storage.local.get(["data", "enabled", "latestUpdate"], (items = {})=>{
...
setInterval(()=>{
    chrome.storage.local.get(["timer", "latestUpdate"], (elems = {})=>{
        const {timer = -1, latestUpdate = 0} = elems;
        if ((timer|0)=== -1) {
            chrome.storage.local.set({"latestUpdate": g_latestUpdate});
            return;
        }
        ...
        if (currentTime - latestUpdate > (timer|0) * 60 * 1000){
            generateNewFingerPrint()
                .then((generatedHash)=>{
                    ...
                    return chrome.storage.local.set({"latestUpdate": g_latestUpdate});
                })
                .then(_=>{
...
function generateNewFingerPrint() {
    return new Promise((success, fail)=>{
        ...
        chrome.storage.local.set({"data": jsonData, "latestUpdate": g_latestUpdate}, ()=>{
...
function notifyUser(title, message, url) {
    ...
    if (url) {
        if (needShow(url)) {
            chrome.notifications.create(`canvas-generate-${getRandomString()}`, options, (notificationId)=>{
                setTimeout(()=>{
        ...
        chrome.notifications.create("canvas-generate", options, (notificationId)=>{});
    }
}