	...
	function doTop() {
		const frame = {
			...
			sendIframeList: function() {
				...
				while (item.index--) {
					({origin, source} = frame.iframeList[item.index]);
					if (origin === "null") {
						origin = "*";
					}
					try {
			...
			windowMessages: function({origin, source, data}) {
				let item, indx;
				...
				case "windowUnloaded":
					...
					break;
				case "contentLoaded":
					if ((source || {}).top !== window) return;
					item = {origin: origin, source: source, id: data.id};
					indx = frame.iframeList.findIndex(({id}) => id === data.id);
					if (indx == -1) {
